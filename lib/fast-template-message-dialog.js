...
export default class FastTemplateMessageDialog extends React.Component {
  ...
  transformTemplate() {
    const promises = this.state.templateNotes.map(async doc => {
      ...
      if (!metadata.title) {
        throw new Error("metadata should have title property");
      }
      ...
      if (Object.hasOwn(metadata, 'date')) newMetadata.date = date
      if (Object.hasOwn(metadata, 'time')) newMetadata.time = time

      const yamlStr = `---\n${yaml.dump(newMetadata)}---\n\n`;

      const body = yamlStr + await engine
